...
// BFS로 최단 경로 찾기
bool LaiserAttack(Turret start, Turret dst){
    final_path = "";
    ...
    while(!qu.empty()){
        ...
        for(int d = 0 ; d < 4 ; d++){
            ...
            if(map[new_x][new_y] == 0) continue;    // 부서진 포탑 위치는 지나갈 수 X
    ...
    if(final_path.size() != 0){
        ...
        for(int i = 0; i < final_path.size(); i++){
            ...
            // 공격력 업데이트
            tmp[next_x][next_y] -= attack_point;
        }
        return true;
    }
    else return false;
...
int main(){
    ...
    while(turn <= K && turrets.size() > 1){
        ...
        attack_point = tmp[attacking.x][attacking.y] / 2;
        if(attack_point < 1) continue;

        // 2. 레이저 공격 가능한지
        // 3. 안되면 포탄 공격
        if (!LaiserAttack(attacking, attacked)) {
